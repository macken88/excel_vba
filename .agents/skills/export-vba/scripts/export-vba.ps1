...
if ($checkText -match "seems to be disabled") {
    throw "Excel VBA Trust Access is not enabled. Export stopped before reading any workbook."
}
...
if ($requestedWorkbooks.Count -eq 0) {
    throw "No workbook exports were resolved from the provided BookFiles arguments."
}
...
$requestedWorkbooks | ForEach-Object {
    Write-Host "Exporting with config: $($_.ConfigName)"
    $exportResult = Invoke-External -FilePath $excelVbaExe -ArgumentList @(
        "export",
        "--file", $_.File,
        "--vba-directory", $_.VbaDirectory
    )
    $exportResult.Output | ForEach-Object { Write-Host $_ }

    if ($exportResult.ExitCode -ne 0) {
        throw "Export failed for workbook $($_.File) with config $($_.ConfigName)"
    }
}

Write-Host "Export completed for all requested workbooks."
